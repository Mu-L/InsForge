...
<CardGroup cols={2}>
  ...
  <Card title="No BYOK in the dashboard" icon="key">
    `FLY_API_TOKEN` is read from the environment only. There's no in-dashboard token entry form (unlike Model Gateway BYOK). If you want to swap tokens, edit `.env` and restart.
  </Card>
</CardGroup>
