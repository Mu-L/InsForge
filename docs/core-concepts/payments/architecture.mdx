...
The `payments` schema stores Stripe connection metadata, catalog mirrors, customer mirrors, checkout attempts, subscriptions, payment history, and webhook processing state.
...
| `payments.customers` | Mirrored Stripe customers for admin visibility and debugging |
